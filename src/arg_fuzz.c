...
int enable_dependency(arg_node* arg_array, ir* ir_, int id) {
  ...
  // if (ir_[id].enable == 1 && dependency_id_len != 0 && child_id_len == 0) {
  if (ir_[id].enable == 1 && dependency_id_len != 0) {
    for (i = 0; i < arg_array[id].dependency_id_len; i++) {
      parent_id = arg_array[id].dependency_id[i];
      if (ir_[parent_id].enable == 0) {
        // if (!strcmp(arg_array[parent_id].arg, "-c"))
          // printf("%s[%d] enables %s[%d] \n",arg_array[id].arg, id, arg_array[parent_id].arg, parent_id);
...
int enable_dependant(arg_node* arg_array, ir* ir_, int id) {
  ...
  if (ir_[id].enable == 1 && child_id_len != 0 && arg_array[id].must_child) {
